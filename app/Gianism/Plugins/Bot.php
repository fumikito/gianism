...
class Bot extends PluginBase {
	...
	public function register_post_type() {
		/**
		 * Passed register_post_type
		 *
		 * @filter gianism_twitter_bot_post_type_args
		 *
		 * @param array $args
		 */
	...
	public function save_post( $post_id, $post ) {
		...
		// Save end date time
		update_post_meta( $post->ID, $this->limit_key, $this->input->post( 'tweet_ends' ) );
	...
	private function clear_schedule( $post ) {
		...
		$query = <<<SQL
			...
			WHERE post_id = %d
			  AND meta_key LIKE %s
SQL;
		$this->db->query( $this->db->prepare( $query, $post->ID, $this->time_key . '_%' ) );
	}
	...
	public function execute_cron() {
		// Clea outdated.
		$query           = <<<SQL
			SELECT p.ID FROM {$this->db->posts} AS p
			INNER JOIN {$this->db->postmeta} AS pm
			ON p.ID = pm.post_id
			WHERE p.post_type = %s
...
SQL;
		...
		$query  = <<<SQL
			...
			WHERE p.post_status = 'publish'
			  ...
			  AND pm.meta_value >= %s
SQL;
		$posts  = $this->db->get_results( $this->db->prepare( $query, $this->post_type, $meta_key, $now, $before ) );
